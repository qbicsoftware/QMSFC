...
my.splot.sample.mcia <-
  function(x, axis1=1, axis2=2, 
    ...
    if (length(col) == 1) {
      ...
      col <- rep(col, ndata)
    #### MODIFICATIONS ####
    col.pal <- brewer.pal(n = 9, name = 'Set1')
    col.types <- unique(col)
    col.final <- c()
    for (i in 1:length(col.types)) {
      col.len <- length(which(col == col.types[i]))
      for (j in 1:col.len) {
        col.final <- c(col.final, col.pal[i])
      }
    }
    col <- col.final
    cl <- col.pal
    ...
    if (legend && any(c(length(cl) != 1, length(pl) != 1, !is.null(phenovec)))) {
      ...
      if (length(cl) == nrow(syn)) {
        ...
        cle <- unique(as.character(phenovec))
        #### END ANOTHER MODIFICATION++++
      }
...
my.plot.mcia <-
  function(x, axes=1:2, sample.lab=TRUE, sample.legend=TRUE, sample.color=1, phenovec=NULL, df.color=1, df.pch=NA, gene.nlab=0, ...) 
  {
    ...
    my.splot.sample.mcia(mcoin, axis1=axes[1], axis2=axes[2], 
    ...
    omicade4:::splot.mol.mcia(mcoin, axis1=axes[1], axis2=axes[2], col=df.color, pch=pch, gene.nlab=gene.nlab)
